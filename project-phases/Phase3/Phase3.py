...
def get_temperature():
    ...
    if not sensebox_data or (datetime.utcnow() - sensebox_data.get("timestamp", datetime.min)) > timedelta(hours=1):
        ...
        data = response.json()
        temperatures = [box["sensors"][0]["lastMeasurement"] if "sensors" in box and box["sensors"][0]["type"] == "temperature" else None for box in data if box]
        valid_temperatures = [temp for temp in temperatures if temp is not None]
        avg_temperature = sum(valid_temperatures) / len(valid_temperatures) if valid_temperatures else 0
